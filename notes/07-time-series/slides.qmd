...
+ midterm [review](https://github.com/edrubin/EC421S26#midterm-exam) (Tuesday; bring questions)
+ midterm
...
- Sampled _across_ a population (_e.g._, people, counties, countries).
- Sampled at _one moment_ in time (_e.g._, Jan. 1, 2015).
- We had $n$ _individuals_, each indexed $i$ in $\left\{1,\,\ldots,\, n \right\}$.
...
- Sampled _within_ [one unit/individual]{.pink } (_e.g._, Oregon).
...
[(We already know that we can relax the last two assumptions.)]{.it .slab .grey-vlight }
...
The outcome can depend on [explanatory variable(s) in other periods]{.purple }.
...
_In other words:_ Births today depend today's income and *lags* of income—_e.g._, last month's income, last year's income, ...
...
The outcome depends on the [explanatory variable(s) and/or the outcome variable in prior periods]{.purple }.
...
By including just **one lag of the dependent variable**—as in a ADL(1, 0)—we implicitly include for *many lags* of the explanatory variables and disturbances. ^[These lags enter into the equation in a very specific way---not the most flexible specification.]
...
1. The disturbance $u_t$ is independent of the explanatory variables<br>in the **same period** (_i.e._, $X_t$).

1. The disturbance $u_t$ is independent of the explanatory variables<br>in the **other periods** (_i.e._, $X_s$ for $s\neq t$).
...
_I.e._, to guarantee the numerator equals zero, we need $\mathop{\boldsymbol{E}}\left[ u_t | X \right] = 0$,<br>for both $\mathop{\boldsymbol{E}}\left[ u_t | X_t \right] = 0$ *and* $\mathop{\boldsymbol{E}}\left[ u_t | X_{s} \right] = 0$ $(s\neq t)$.
...
## Autoregressive distributed-lag models {visibility="uncounted"}
### Equilibrium effects
...
So long as $-1<\beta_2<1$.^[This simplification comes from $\sum_{k=0}^\infty p^k = \dfrac{1}{p}$ for $-1<k<1$.]
