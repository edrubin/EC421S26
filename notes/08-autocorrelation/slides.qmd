...
In [R]{.mono }, you use the `function()` function to write functions.^[Meta.]
...
ylab("u") +
xlab("t") +
# theme_pander(base_family = "Fira Sans Condensed", base_size = 20)
theme_axes_math
```
...
<br>[(Correlated disturbances are actually what we have with autocorrelation.)]{.it .slab .grey-vlight }
...
[**Problem:** $\text{Births}_{t-1}$ (a regressor in the model for time $t$) and<br>$u_{t}$ (the disturbance for time $t$) both depend upon $u_{t-1}$.<br>*I.e.*, a regressor is correlated with its contemporaneous disturbance.]{.fragment}


[**Q:** Why is this a problem?]{.fragment}

[**A:** It violates [contemporaneous exogeneity]{.pink }]{.fragment}[, *i.e.*, $\mathop{\text{Cov}} \left( x_t,\,u_t \right) \neq 0$.]{.fragment}
...
we cannot separate the effects of $u_t$ on $\text{Births}_t$ from $\text{Births}_{t-1}$ on $\text{Births}_{t}$, because both $u_t$ and $\text{Births}_{t-1}$ depend upon $u_{t-1}$. [$\textcolor{#e64173}{\hat{\beta}_2}$ [is **biased** (w/ OLS).]{.pink }]{.fragment}
:::
...
For an example, let's consider the relationship between monthly presidential approval ratings and oil prices during President George W. Bush's^[[Fun with approval ratings](https://projects.fivethirtyeight.com/trump-approval-ratings/).] presidency.

We will 

1. specify the process as ADL(1, 0),
2. test for an AR(2) process in our disturbances.
...
::: {.fragment}
[Note:]{.note} We're ignoring any other violations of exogeneity for the moment.
:::
...
ggplot(data = approval_df, aes(x = time, y = ap)) +
  ...
  xlab("Date") +
  ylab("Rating") +
  theme_minimal(base_family = "Fira Sans Condensed", base_size = 20)
```
...
ggplot(data = approval_df, aes(x = lag(approve), y = ap)) +
  geom_smooth(method = lm, se = FALSE, linetype = "dashed", linewidth = .8, color = "darkslategrey") +
  geom_point(size = 2.5, color = red_pink) +
  xlab("Previous") +
  ylab("Current") +
  theme_minimal(base_family = "Fira Sans Condensed", base_size = 20)
...
ggplot(data = approval_df, aes(x = price_oil, y = ap)) +
  geom_smooth(method = lm, se = F, linetype = "dashed", linewidth = .8, color = "darkslategrey") +
  geom_point(size = 2.5, color = red_pink) +
  xlab("Oil price ($)") +
  ylab("Rating") +
  theme_minimal(base_family = "Fira Sans Condensed", base_size = 20)
...
[Approval rating vs. oil prices, 2001–2006]{.b .slate } [with quadratic fit]{.grey-vlight }
```{r}
#| label: "approval-6"
#| echo: false
#| fig-width: 12
...
ggplot(data = approval_df, aes(x = price_oil, y = ap)) +
  geom_smooth(method = lm, formula = y ~ x + I(x^2), se = F, linetype = "dashed", linewidth = .8, color = "darkslategrey") +
  geom_point(size = 2.5, color = red_pink) +
  xlab("Oil price ($)") +
  ylab("Rating") +
  theme_minimal(base_family = "Fira Sans Condensed", base_size = 20)
...
::: {.fragment}
[{[?]{.pink }, 1, 2, 3, 4, 5, 6, 7, 8} = lag(x)]{.mono }
:::

::: {.fragment}
[{[?]{.pink }, [?]{.pink }, 1, 2, 3, 4, 5, 6, 7} = lag(x, 2)]{.mono }
:::

::: {.fragment}
[{[?]{.pink }, [?]{.pink }, [?]{.pink }, 1, 2, 3, 4, 5, 6} = lag(x, 3)]{.mono }
:::
...
With a *p*-value of $\sim$ `r waldtest(bg_reg, c("lag(e)", "lag(e, 2)"))$"Pr(>F)"[2] %>% round(3)`, [we fail to reject the null hypothesis.]{.b .pink }
...
I'll give you three options.^[You should take [EC 422]{.mono } to go much deeper into time-series analysis/forecasting.]
...
<br>[As we did with the heteroskedasticity-robust standard errors.]{.it .grey-vlight } 

These standard errors are called [serial-correlation robust standard errors]{.b .purple} (or *Newey-West standard errors*).^[We are not going to derive the estimator for these standard errors.]

::: {.fragment}
In `R`, we can use `fixest`'s `feols` function to estimate these standard errors.
```{r}
#| label: "nw1"
#| echo: true
#| eval: false
...
fixest::feols(
  approve ~ l(approve) + price_oil,
  data = approval_df,
  panel.id = ~ country + time_months,
  se = "NW"
)
```
:::
...
# Compare the two sets of standard errors from fixest
approval_df = arrange(approval_df, time)
approval_df =
  approval_df |>
  mutate(
    country = 'usa',
...
# Estimate the model with OLS standard errors
est_iid =
  fixest::feols(
    approve ~ l(approve) + price_oil,
    data = approval_df,
...
:::
::: {.column width="50%"}
[`l` denotes a _lag_ in `fixest`.]{.fragment fragment-index=0}

[`panel.id` identifies the individual and time dimensions of our data.]{.fragment fragment-index=1}

[`se = 'NW'` for _Newey-West_ stnd. errors.]{.fragment fragment-index=2}
:::
:::
...
# Compare the two sets of standard errors from fixest
fixest::etable(est_iid, est_nw)
```

The _p_-values on the price of oil coefficient: 0.062 vs. 0.023.
...
## Autocorrelation
### _Summary:_ Main ideas

[Autocorrelation]{.b .pink } means the disturbances correlated through time,
  $$\mathop{\text{Cov}}\left(u_t, u_s\right) \neq 0 \quad \text{for } t \neq s.$$

- For **static time-series models**, OLS _coefficient_ estimates are [unbiased]{.b .pink }^[Assuming exogeneity],<br>but OLS _standard errors_ are [biased]{.b .pink } and OLS is [inefficient]{.b .pink }.

- For **lagged outcome variables**, OLS coefficient estimates<br>are [biased and inconsistent]{.b .orange }.
...
## Autocorrelation
### _Summary:_ Testing/living with autocorrelation

Tests for autocorrelation involve regressing residuals on their lags.

- In **static models**: regress residuals on their lags.

- In **dynamic models with lagged outcomes**: use [Breusch-Godfrey]{.b .purple }<br>regress residuals on the original regressors _plus_ lagged residuals.

Potential solutions/responses (akin to _heteroskedasticity_):

- Consider whether the model is [misspecified]{.b .pink }.

- Use [Newey-West]{.b .pink } standard errors.

- For static models with AR disturbances, [FGLS]{.b .pink } can transform the model to remove autocorrelation.
